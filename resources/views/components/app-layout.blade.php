...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="flex h-screen overflow-hidden bg-paper font-sans antialiased text-ink dark:bg-paper-dark dark:text-[#F0E6D8]" x-data="{ sidebarOpen: false }">
        <div x-show="sidebarOpen" x-cloak class="fixed inset-0 z-20 bg-ink/40 lg:hidden" x-transition.opacity @click="sidebarOpen = false"></div>

        @include('layouts.sidebar')

        <div class="app-content-shell relative flex min-w-0 flex-1 flex-col overflow-hidden">
            <div class="no-print">
